...
你需要掌握C++ `std::string`的**全部成员函数+全局重载函数**，包含完整的函数分类、语法、功能说明、使用示例、核心注意事项，同时补充所有高频使用细节和易错点，**涵盖C++98~C++20常用函数，无遗漏全覆盖**，学完这篇即可精通string所有用法。
> 前置必看：使用所有`std::string`函数前，必须引入头文件 `#include <string>`，所有示例默认声明 `using namespace std;`，`size_t` 是无符号整数类型（unsigned int 别名），`string::npos` 是静态常量，值为无符号最大值，代表「无匹配/到末尾」。
...
## 总结（核心要点速记，必背）
1. `std::string` 是字符的动态数组，底层自动管理内存，无需手动释放，支持所有字符串操作；
2. 访问字符：`[]` 无检查效率高，`at()` 有检查更安全，`front()/back()` 快捷访问首尾；
3. 追加内容优先用 `+=/push_back()`，修改内容优先用 `replace()`，查找用 `find()`+`npos` 判断；
4. 读取带空格的字符串必须用 `getline()`，`cin>>` 只读取无空格字符串；
5. C++11的 `pop_back()、emplace_back()`、移动构造/赋值，都是性能优化的核心，必须使用；
6. 所有查找函数返回 `size_t`，判断未找到的标准写法是 `== string::npos`。

至此，`std::string`的所有函数已全部详解完毕，涵盖所有语法、示例、注意事项，这篇内容足够应对所有开发场景和面试考点！
